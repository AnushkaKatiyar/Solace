...
if st.session_state.project_type == "new":
    ...
    if "final_plan" in st.session_state and st.session_state.final_plan is not None:
        ...
        st.markdown(
            """
            <div style="margin: 30px 0 20px 0; font-size: 18px; color: grey; font-style: italic; display: flex; align-items: center;">
                <span style="font-size: 18px; margin-right: 8px;">ℹ️</span>
                <span><b>Note:</b> These predictions are generated by a machine learning model trained on historical school construction data. 
                The model has a Root Mean Square Error (RMSE) of ±$651,000 and explains about 98.8% of the variation in cost (R² = 0.988). 
                Actual costs may vary depending on project-specific factors not captured in the data.</span>
            </div>
            """,
            unsafe_allow_html=True
        )
